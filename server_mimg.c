...
// sudo docker run -it --rm -p 8001:8001 -v .:/workspace c-assignment-env
...
sem_t array_mutex;
sem_t socket_mutex;
sem_t * img_sem_array = NULL;


struct img_array *waiting_operations; //Global list, keeps track of how many requests per image 
//waiting for an operation and their ids
sem_t * requests_mutex;
...
int add_to_queue(struct request_meta to_add, struct queue * the_queue)
{
    ...
    if (the_queue->available == 0) {
        ...
        struct img_array *img_arr = &waiting_operations[to_add.request.img_id];
        if (img_arr->count == img_arr->limit) {
            img_arr->limit = (img_arr->limit == 0) ? 1 : img_arr->limit * 2;
            img_arr->req_ids = realloc(img_arr->req_ids, img_arr->limit * sizeof(uint64_t));
        }
        img_arr->req_ids[img_arr->count++] = to_add.request.req_id;
...
void register_new_image(int conn_socket, struct request * req)
{
    ...
    img_sem_array = realloc(img_sem_array, image_count * sizeof(sem_t));

    sem_wait(requests_mutex);

    waiting_operations = realloc(waiting_operations, image_count * sizeof(struct img_array));
    waiting_operations[image_count - 1].req_ids = NULL;
    waiting_operations[image_count - 1].count = 0;
    waiting_operations[image_count - 1].limit = 0;
    ...
    sem_init(&img_sem_array[image_count - 1], 0, 1);
...
int worker_main (void * arg) {
    ...
    while (!params->worker_done) {
        ...
        sem_wait(&img_sem_array[img_id]); 

        struct img_array *img_arr = &waiting_operations[img_id];

        while (img_arr->count > 0 && img_arr->req_ids[0] != req.request.req_id) {
            sem_post(&img_sem_array[img_id]); 
            sem_wait(&img_sem_array[img_id]); 
        }

        sem_wait(requests_mutex);
        memmove(img_arr->req_ids, img_arr->req_ids + 1, (img_arr->count - 1) * sizeof(uint64_t));
        img_arr->count--;
        sem_post(requests_mutex);
        ...
        switch (req.request.img_op) {
            case IMG_ROT90CLKW:
                ...
                break; //No need for semaphores here because funcitons return pointers to a newly created instances of images
            case IMG_BLUR:
                img = blurImage(img);
                break;
            case IMG_SHARPEN:
        ...
        if (req.request.img_op != IMG_RETRIEVE) {
            ...
            sem_post(&array_mutex); //Release the right for the gloval array of images
        }
        sem_post(&img_sem_array[img_id]); 
    ...
    do {
        ...
        if (in_bytes > 0) {
            ...
            if (res) {
                ...
                sem_wait(&socket_mutex);
                send(conn_socket, &resp, sizeof(struct response), 0); //atomic operation
                sem_post(&socket_mutex);
...
int main (int argc, char ** argv) {
	...
	retval = sem_init(&socket_mutex, 0, 1);
 	...
 	requests_mutex = malloc(sizeof(sem_t));
 	if (sem_init(requests_mutex, 0, 1) != 0) {
 		perror("Unable to initialize requests_mutex");
 		return EXIT_FAILURE;
 	}
	sem_init(&array_mutex, 0, 1);
 	img_sem_array = NULL; 
 	/* Handle connection */
	...
	if (images) {
 	...
 	if (waiting_operations) {
 		for (size_t i = 0; i < image_count; i++) {
 			if (waiting_operations[i].req_ids) {
 				free(waiting_operations[i].req_ids);
 			}
 		}
 		free(waiting_operations);
 	}
